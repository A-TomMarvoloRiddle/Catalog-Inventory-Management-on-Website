...
if (isset($_POST['action'])) {
    ...
    switch ($action) {
        ...
        case "searchByFeature":
            ...
            $featr = $extra;
            $sql = "SELECT * FROM product WHERE featr LIKE '%$featr%' ORDER BY pno";
            $result = $conn->query($sql);
            $table = "Search Results:\n---------------------------------------------------------\n";
            if ($result && $result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
        ...
        case "getTotalCount":
            // Return the total count of products as a JSON value for auto-filling Product Number.
            $sql = "SELECT * FROM product";
            $result = $conn->query($sql);
            $total = ($result) ? $result->num_rows : 0;
  ...
  <script>
    ...
    function adminMenuHTML() {
      return `
        ...
        <button onclick="ajaxAction('total')">Total No. of Items</button>
